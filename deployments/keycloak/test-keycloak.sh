...
ACCESS_TOKEN=$(curl --silent -d 'scope=openid' -d 'client_id='${KEYCLOAK_CLIENT_NAME} -d 'client_secret='${KEYCLOAK_CLIENT_SECRET} -d 'username='${USER_EMAIL_01} -d 'password='${USER_PASSWORD_01} -d 'grant_type=password' http://localhost:18080/realms/${KEYCLOAK_REALM}/protocol/openid-connect/token | jq -r .access_token)
if [ -z "${ACCESS_TOKEN}" ] || [ "${ACCESS_TOKEN}" == "null" ]; then
	curl -d 'scope=openid' -d 'client_id='${KEYCLOAK_CLIENT_NAME} -d 'client_secret='${KEYCLOAK_CLIENT_SECRET} -d 'username='${USER_EMAIL_01} -d 'password='${USER_PASSWORD_01} -d 'grant_type=password' http://localhost:18080/realms/${KEYCLOAK_REALM}/protocol/openid-connect/token
